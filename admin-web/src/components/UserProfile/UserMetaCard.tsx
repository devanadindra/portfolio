...
import { addAvatar } from "../../services/userService";
...
export default function UserMetaCard({ data, refetch }: UserMetaCardProps) {
  if (!data) return null;
  // useModal sekarang hanya untuk status buka/tutup
  ...
  return (
    <>
      ...
      >
        <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex flex-col items-center w-full gap-6 xl:flex-row">
            ...
            {/* Info Teks */}
            <motion.div variants={itemVariants} className="order-3 xl:order-2">
      ...
      <Modal isOpen={isOpen} onClose={closeModal} className="max-w-md m-4">
        {/* Menggunakan AnimatePresence untuk animasi keluar/masuk saat isOpen berubah */}
        <AnimatePresence>
            {isOpen && (
                <motion.div
                    key="modal-content"
                    ...
                    animate="visible"
                    exit="exit"
                    className="no-scrollbar relative w-full max-w-md overflow-y-auto rounded-3xl"
                >
                    <div className="px-2">
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
                            Choose a new profile photo for your account.
                        </p>
                    </div>
                    ...
                    <form
                        ...
                        onSubmit={async (e) => {
                            ...
                            try {
                                setLoading(true);
                                const res = await addAvatar(selectedFile);
                                ...
                                if (!res.errors) {
                                    ...
                                    showAlert("error", "Upload Failed", "Gagal upload avatar!");
                                }
                            } catch (err) {
                                console.error(err);
                                showAlert("error", "Upload Failed", "Terjadi kesalahan");
                            } finally {
                                setLoading(false);
                            }
                        }}
                    >
                        <motion.img
                            ...
                            alt="preview"
                            className="h-50 w-50 rounded object-cover border border-gray-300"
                        />
                        ...
                        <Button size="sm" type="submit">Upload Avatar</Button>
                    </form>
                    {/* Akhir Konten Modal Asli */}
                </motion.div>
            )}
        </AnimatePresence>
      </Modal>
